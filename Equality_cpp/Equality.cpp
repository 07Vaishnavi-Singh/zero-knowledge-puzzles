...
void Equality_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Equality_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "Check1";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "Check2";
IsEqual_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 6 ;
aux_cmp_num += 2;
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 29
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 2; i++){
...
Equality_2_create(1,0,ctx,"main",0);
Equality_2_run(0,ctx);
}
